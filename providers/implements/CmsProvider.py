...
class CmsProvider(ProviderInterface):
  ...
  def getGameDetail(self, params):
    """
    查询游戏战局详情
    """
    return cmsapi.getHistoryGameDetail(
      self.apiUsername,
      self.apiPwd,
      params['room_id']
    )
  ...
  def queryUserBoard(self, params):
    ...
    for key, item in rel['result']['list']:
      # get detail
      detail = cmsapi.getHistoryGameDetail(
        self.apiUsername,
        self.apiPwd,
        # self.clubId,
        item['roomid']
