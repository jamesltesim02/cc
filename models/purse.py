...
def getPurseWithCursor(cursor, gameId):
    sql = """
        select * 
        from
          onethink_ucenter_vid_member as uv,
          onethink_player_purse as pp,
          onethink_ucenter_member as um
        where
          uv.game_vid=%s
          and
          uv.frontend_user_auth = pp.username
          and
          uv.frontend_user_auth = um.username
    """
    cursor.execute(sql, (gameId))
    return cursor.fetchone()
...
def syncBuyin(conn, purseInfo, buyin, delta):
    ...
    try:
        ...
        clubRoomName = base64.b64encode(
            (buyin['club_name']+'_'+buyin['room_name']).encode('utf-8'))
